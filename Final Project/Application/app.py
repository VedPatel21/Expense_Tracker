...
API_KEY = '66643b7d001f84fc89074a7d'
        ...
        try:
            ...
            if recurring:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (user_id, title, amount, category, date, frequency))
            else:
                conn.execute("""
...
# Add route to handle recurring expenses (to be run periodically, e.g., with a cron job)
def process_recurring_expenses():
    """
    Processes recurring expenses and adds them to the expenses table on their respective due dates.
    This function is meant to be run periodically, for example with a cron job.
    """
    current_date = datetime.now().date()
    recurring_expenses = db.execute("SELECT * FROM recurring_expenses")

    for expense in recurring_expenses:
        last_date = datetime.strptime(expense['start_date'], '%Y-%m-%d').date()
        while last_date < current_date:
            next_date = last_date + timedelta(days=30) if expense['frequency'] == 'monthly' else last_date + timedelta(weeks=1)
            if next_date <= current_date:
                db.execute("INSERT INTO expenses (user_id, title, amount, category, date) VALUES (?, ?, ?, ?, ?)",
                           expense['user_id'], expense['title'], expense['amount'], expense['category'], next_date.strftime('%Y-%m-%d'))
                last_date = next_date
            else:
                break
    ...
    if request.method == 'POST':
        ...
        if file and file.filename.endswith('.csv'):
            ...
            for index, row in df.iterrows():
                db.execute("INSERT INTO expenses (user_id, title, amount, date, category) VALUES (?, ?, ?, ?, ?)",
...
def expense_data():
    ...
    user_id = session['user_id']
    expenses = db.execute("SELECT date, amount, category FROM expenses WHERE user_id = ?", user_id)
